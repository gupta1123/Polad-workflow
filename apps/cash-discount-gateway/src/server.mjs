  ...
  if (bankMatchingNeedsConnectorUpdate(operation, message.payload, authorized.bridgeVersion)) {
    send(socket, { type: "result", requestId, success: false,
      error: operation === "local_ledger_catalogue"
        ? "Install Polaad Tally Connector 0.1.70 or later to use the local ledger catalogue."
        : "Install Polaad Tally Connector 0.1.59 or later on the Tally PC before checking matches. Older connectors use a heavy bill scan that can freeze Tally." });
    return;
  }
  if (pending.has(requestId)) {
