...
function getPythonCandidates(): PythonCandidate[] {
  const configured = process.env.POLAAD_PDF_PYTHON_BIN?.trim();
  ...
  try {
    ...
    if (result.ok && result.decrypted) {
      if (!fs.existsSync(outputPath)) {
        throw new PdfSecurityError(
          "BANK_STATEMENT_PDF_INVALID",
          "Polaad unlocked the statement but could not create a readable PDF. Export the statement again and retry."
        );
      }
    ...
    if (result.code === "encryption_unsupported") {
      throw new PdfSecurityError(
        "BANK_STATEMENT_PASSWORD_UNSUPPORTED",
        "Polaad cannot open this PDF's encryption type. Remove its security in your PDF viewer, export a new PDF, and upload it again."
      );
    }
