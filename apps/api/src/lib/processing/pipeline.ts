...
const CONSIGNEE_EXTRACTION_INSTRUCTION =
  "If a party is labelled Consignee, Ship To, or Recipient, map it to buyerName/buyerGstin only when that party is Polaad or Polaad Steel Alloys. If the consignee/ship-to/recipient is any other party, do not use that value as buyerName or buyerGstin; keep it only in visibleText or address fields when applicable. ";
...
function normalizeCompanySignal(value: string) {
  return value
    ...
    .filter(Boolean)
    .map((token) => {
      if (["kaliko", "xarika", "xarixa"].includes(token)) return "polaad";
      return token;
    })
    .join(" ");
}
...
function isInternalConsigneeName(value?: string) {
  return Boolean(value && normalizeCompanySignal(value).split(/\s+/).includes("polaad"));
}
      ...
      [
        {
          ...
          content:
            ...
            "For consignee/ship-to/recipient, buyerName/buyerGstin may be Polaad only; non-Polaad consignee values must not become buyer fields. " +
            "For GST, preserve visible tax percentages separately from tax amounts. Same Maharashtra/Polaad state code 27 means CGST+SGST split unless the document explicitly shows IGST. " +
...
function cleanEWayPartyName(value?: string, knownGstin?: string) {
  ...
  if (/\bpolaad\b/i.test(cleaned) && !/^polaad\b/i.test(cleaned)) return undefined;
...
function splitEWayPartyPair(value?: string): Partial<Record<"vendorName" | "buyerName", string>> {
  ...
  const polaadMatch = candidate.match(/\bPOLAAD\b/i);
  if (polaadMatch?.index && polaadMatch.index > 0) {
    return {
      vendorName: cleanEWayPartyName(candidate.slice(0, polaadMatch.index)),
      buyerName: cleanEWayPartyName(candidate.slice(polaadMatch.index)),
    };
  }
      ...
      [
        {
          ...
          content:
            ...
            "For seller-issued documents, vendorName is the issuing supplier/seller/consignor and buyerName is the receiving buyer, excluding non-Polaad consignee/ship-to/recipient parties. " +
    ...
    [
      {
        ...
        content:
          ...
          lineItemInstruction +
          STAMP_SIGNATURE_EXTRACTION_INSTRUCTION +
          "Use only information present in the visible text. For seller-issued documents, vendorName is the issuing supplier and buyerName is the receiving buyer, excluding non-Polaad consignee/ship-to/recipient parties. " +
          "For Purchase Order or Amended Purchase Order documents, vendorName is the supplier/vendor receiving the order and buyerName is the purchaser issuing the order.",
      },
      {
        role: "user",
