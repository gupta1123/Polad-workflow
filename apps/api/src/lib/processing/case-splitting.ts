...
function isPolaadFacingDocument(document: CaseDoc) {
  return POLAAD_PATTERN.test(
...
function attachSellerChainContextToPrimaryShipments(
  ...
  const primaryIndexes = groups
    .map((group, index) => (isPolaadFacingShipmentGroup(group) ? index : -1))
  ...
  for (let index = 0; index < groups.length; index += 1) {
    ...
    // Polaad-facing shipment. Keeping the original packet is safer than losing context.
...
function shouldSplitGroups(groups: ShipmentGroup[], documents: CaseDoc[]) {
  ...
  // been attached to one Polaad-facing shipment by source-file evidence.
  if (hasSellerChain(documents) && groups.some((group) => !isPolaadFacingShipmentGroup(group))) {
    return false;
  }
