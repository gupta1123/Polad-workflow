...
type InvoiceGroup = {
  ...
  isPolaadFacing: boolean;
};

const POLAAD_PATTERN = /\bpolaad\b|polaad\s+steel|polaad\s+alloys/i;
...
function buildInvoiceGroups(documents: PacketDocumentLike[]) {
  ...
  return Array.from(groups.entries()).map(([key, groupDocuments]): InvoiceGroup => {
    ...
    return {
      ...
      isPolaadFacing: POLAAD_PATTERN.test(`${buyerName ?? ""} ${readField(first, ["buyerGstin"]) ?? ""}`),
    };
  });
}
...
function buildShipmentGroups(
  ...
  return invoiceGroups.map((group, index): PacketShipmentGroup => {
    const isContext = sellerChain && !group.isPolaadFacing;
    const role = sellerChain
      ? group.isPolaadFacing
        ? "primary"
        : "context"
...
export function resolvePacketIntelligence({
  ...
  const polaadFacingGroups = uniqueInvoiceGroups.filter((group) => group.isPolaadFacing);
  const nonPolaadFacingGroups = uniqueInvoiceGroups.filter((group) => !group.isPolaadFacing);
  const sellerChain =
    uniqueInvoiceGroups.length > 1 &&
    polaadFacingGroups.length > 0 &&
    nonPolaadFacingGroups.length > 0;
  ...
  if (duplicateUpload) {
    ...
    tone = "warning";
    confidence = "medium";
    summary = `${polaadFacingGroups.length} Polaad-facing invoice${polaadFacingGroups.length === 1 ? "" : "s"} and ${nonPolaadFacingGroups.length} upstream invoice${nonPolaadFacingGroups.length === 1 ? "" : "s"} found.`;
    recommendedAction = "Reconcile only the Polaad-facing invoice; keep upstream invoices as context.";
  ...
  const checks: PacketIntelligenceCheck[] = [
    ...
    },
    {
      ...
      detail: sellerChain
        ? "Seller-chain documents detected; choose Polaad-facing invoice as primary."
  ...
  const signals: PacketIntelligenceSignal[] = [
    ...
    { label: "Polaad-facing invoices", value: String(polaadFacingGroups.length), tone: sellerChain ? "warning" : "neutral" },
  ];
