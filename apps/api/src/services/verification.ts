...
function getSellerChainRoleSelection(
  ...
  return {
    ...
    note: "Seller-chain role selection applied: Polaad-facing invoice was reconciled; upstream seller invoices were kept as context.",
  };
}
...
function normalizePartyName(value: string | number | null | undefined) {
  ...
  const tokens = String(value)
    ...
    .map((token) => {
      ...
      if (["kaliko", "xarixa", "xarika"].includes(token)) return "polaad";
