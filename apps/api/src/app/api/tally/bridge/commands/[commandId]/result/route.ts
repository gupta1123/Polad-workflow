  ...
  try {
    ...
    if (isNativeDebitNotePdfExport) {
      ...
      if (!success) {
        ...
        // Older debit notes stored their Polaad reference in tally_voucher_id.
        ...
        if (existingVoucherId && /^\d+$/.test(existingVoucherId) && existingVoucherId !== voucherId.trim()) {
          throw new Error("The exported Tally PDF belongs to a different Debit Note than the one created by Polaad.");
        }
        ...
        if (connectorHash && connectorHash.toLowerCase() !== uploaded.sha256.toLowerCase()) {
          throw new Error("The native Tally PDF changed before it reached Polaad. Export was rejected.");
        }
