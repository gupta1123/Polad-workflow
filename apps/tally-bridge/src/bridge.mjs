...
const PURCHASE_DOCUMENT_UDFS = {
  path: { name: "PolaadSourceDocumentPath", index: 30001 },
  name: { name: "PolaadSourceDocumentName", index: 30002 },
  sha256: { name: "PolaadSourceDocumentSha256", index: 30003 },
  id: { name: "PolaadSourceDocumentId", index: 30004 },
};
...
function buildTallyReadinessXml(companyName) {
  ...
  return [
    ...
    "<TDL>",
    "<TDLMESSAGE>",
    '<COLLECTION NAME="Polaad Ledgers Probe" ISMODIFY="No">',
    "<TYPE>Ledger</TYPE>",
...
async function fetchAvailableCompanies(tallyUrl, activeCompanyName = null) {
  ...
  try {
    const xml = await exportTallyCollection(tallyUrl, {
      collectionName: "Polaad Available Companies",
      tallyType: "Company",
...
function tallyDataRoots() {
  return mergeCompanyNames([
    process.env.POLAAD_TALLY_DATA_ROOT,
...
async function verifyPurchaseVoucherInTally(config, payload = {}) {
  ...
  const xml = await exportTallyCollection(tallyUrl, {
    collectionName: "Polaad Purchase Voucher Verification",
    tallyType: "Voucher",
    fetchFields:
      "Date,EffectiveDate,ReferenceDate,VoucherTypeName,VoucherNumber,Reference,PartyLedgerName,MasterID,AlterID,GUID,Narration,PolaadSourceDocumentPath,PolaadSourceDocumentName,PolaadSourceDocumentSha256,PolaadSourceDocumentId,AllLedgerEntries.*,AllLedgerEntries.BillAllocations.Name,AllLedgerEntries.BillAllocations.BillType,AllLedgerEntries.BillAllocations.BillDate,AllLedgerEntries.BillAllocations.Amount,AllInventoryEntries.*",
    companyName,
...
function toVoucher(block) {
  ...
  return {
    ...
    sourceDocumentPath: getUdfTagText(block, "POLAADSOURCEDOCUMENTPATH"),
    sourceDocumentName: getUdfTagText(block, "POLAADSOURCEDOCUMENTNAME"),
    sourceDocumentSha256: getUdfTagText(block, "POLAADSOURCEDOCUMENTSHA256"),
    sourceDocumentId: getUdfTagText(block, "POLAADSOURCEDOCUMENTID"),
...
async function resolveDebitNoteVoucher(tallyUrl, payload, fallbackCompanyName) {
  ...
  const xml = await exportTallyCollection(tallyUrl, {
    collectionName: "Polaad Debit Note Voucher Lookup",
    tallyType: "Voucher",
    fetchFields:
  ...
  if (requestedReference && !isSameTallyText(voucher.reference, requestedReference)) {
    throw new Error("Tally Debit Note reference does not match the Polaad request.");
  }
  if (requestedParty && !voucherHasLedger(voucher, requestedParty)) {
    throw new Error("Tally Debit Note customer does not match the Polaad request.");
  }
  if (requestedAmount > 0 && !voucherHasAnyAmount(voucher, requestedAmount)) {
    throw new Error("Tally Debit Note amount does not match the Polaad request.");
  }
  return voucher;
}
...
<html>
  ...
  <body>
    <main class="document" data-polaad-voucher-id="${escapeHtml(voucher.masterId)}">
...
async function exportNativeDebitNotePdf(companyName, voucher, requestedParty, renderTallyPrintToPdf) {
  if (typeof renderTallyPrintToPdf !== "function") {
    throw new Error("The desktop Polaad connector must be running to prepare the official Tally PDF.");
  }
...
async function verifyBankTransactionInTally(config, commandPayload = {}) {
  ...
  const xml = await exportTallyCollection(tallyUrl, {
    collectionName: "Polaad Bank Payment Verification",
    tallyType: "Voucher",
    fetchFields:
      "Date,EffectiveDate,VoucherTypeName,VoucherNumber,Reference,Narration,PartyLedgerName,MasterID,AlterID,IsCancelled,AllLedgerEntries.LedgerName,AllLedgerEntries.Amount,AllLedgerEntries.IsDeemedPositive,AllLedgerEntries.BankAllocations.Name,AllLedgerEntries.BankAllocations.InstrumentNumber,AllLedgerEntries.BankAllocations.TransactionName",
    companyName,
...
async function exportTargetedOpenBillXml(tallyUrl, { companyName, ledgerNames, asOfDate, forceTargeted = false }, exportCollection = exportTallyCollection) {
  ...
  for (const [index, batch] of batches.entries()) {
    const ledgerFilterName = "PolaadRequestedBillLedger";
    responses.push(await exportCollection(tallyUrl, {
      collectionName: `Polaad Customer Open Bills ${index + 1}`, tallyType: "Bill",
      fetchFields: "Name,Parent,LedgerName,PartyLedgerName,BillType,TypeOfRef,Date,BillDate,DueDate,VoucherNumber,VoucherTypeName,OpeningBalance,ClosingBalance,Balance,PendingAmount,Amount",
      companyName, dateTo: asOfDate,
...
async function exportTargetedBillEvidenceXml(tallyUrl, { companyName, ledgerNames, dateFrom, dateTo }, exportCollection = exportTallyCollection) {
  ...
  const responses = [];
  for (const [index, batch] of batches.entries()) {
    const ledgerEntryFilterName = "PolaadRequestedPartyEntry";
    const ledgerVoucherFilterName = "PolaadRequestedPartyVoucher";
    responses.push(await exportCollection(tallyUrl, {
      collectionName: `Polaad Customer Bill Evidence ${index + 1}`, tallyType: "Voucher",
    ...
    try {
      const [ledgerXml, groupXml] = await Promise.all([
        exportTallyCollection(tallyUrl, {
          collectionName: "Polaad Bank Ledger Discovery",
          tallyType: "Ledger",
          fetchFields:
        ...
        exportTallyCollection(tallyUrl, {
          collectionName: "Polaad Bank Group Discovery",
          tallyType: "Group",
...
async function collectTallyMasters(config, commandPayload = {}) {
  ...
  fetchMaster("ledgerXml", "ledger", "Polaad Ledgers Sync", ledgerFetchFields);
  fetchMaster("groupXml", "group", "Polaad Groups Sync", "Name,Parent,GUID");
  fetchMaster("stockItemXml", "stock_item", "Polaad Stock Items Sync",
    "Name,Parent,GUID,BaseUnits,OriginalBaseUnits,GSTHSNCode,HSNCode,GSTTaxRate,RateOfTaxCalculation,IsGSTApplicable");
  fetchMaster("unitXml", "unit", "Polaad Units Sync", "Name,GUID,OriginalName,DecimalPlaces,IsSimpleUnit");
  fetchMaster("voucherTypeXml", "voucher_type", "Polaad Voucher Types Sync", "Name,Parent,GUID");
  if (shouldFetch("ledger") && fieldProfile !== "bank_statement") {
    fetches.push(["companyXml", () => exportTallyCollection(tallyUrl, {
      collectionName: "Polaad Company Profile Sync",
      tallyType: "Company",
  ...
  if (fieldProfile === "bank_statement" && shouldFetch("ledger")) {
    ...
    if (bankLedgerNames.length > 0) {
      ...
      const filterName = "PolaadBankStatementLedgerFilter";
      const bankXml = await exportTallyCollection(tallyUrl, {
        collectionName: "Polaad Bank Statement Ledger Details",
        tallyType: "Ledger",
...
async function diagnoseTallyCompanyCli(args) {
  ...
  const companyName = args["company-name"] || config?.companyName || null;
  const xml = await exportTallyCollection(tallyUrl, {
    collectionName: "Polaad Company Diagnostics",
    tallyType: "Company",
    fetchFields:
...
async function findVouchersCli(args) {
  ...
  const xml = await exportTallyCollection(tallyUrl, {
    collectionName: "Polaad Voucher Lookup",
    tallyType: "Voucher",
    fetchFields:
...
async function listVouchersCli(args) {
  ...
  const xml = await exportTallyCollection(tallyUrl, {
    collectionName: "Polaad Voucher List",
    tallyType: "Voucher",
    fetchFields:
      "Date,EffectiveDate,VoucherTypeName,VoucherNumber,Reference,Narration,PartyLedgerName,MasterID,AlterID,IsCancelled,AllLedgerEntries.LedgerName",
    companyName,
