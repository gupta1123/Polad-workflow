...
test("Purchase vouchers use Tally's item-invoice envelope and allocation tags", () => {
  const xml = buildPurchaseVoucherXml({
    ...
    sourceDocumentPath: "C:\\Polaad Documents\\VIS-0142.pdf",
  ...
  assert.match(xml, /<UDF:POLAADSOURCEDOCUMENTPATH\.LIST[^>]*INDEX="30001">/);
  assert.match(xml, /C:\\Polaad Documents\\VIS-0142\.pdf/);
  assert.match(xml, /<UDF:POLAADSOURCEDOCUMENTSHA256[^>]*>ABC123<\/UDF:POLAADSOURCEDOCUMENTSHA256>/);
  assert.match(xml, /<UDF:POLAADSOURCEDOCUMENTID[^>]*>file-1<\/UDF:POLAADSOURCEDOCUMENTID>/);
